...
Fork of [Remnawave Node](https://github.com/remnawave/node) `3.3.2`, built with the
...
| | |
|---|---|
| Node | `3.3.2` (`version` in `package.json`) |
| Xray core | `v26.8.26-0855` from `Jolymmiles/Xray-core` (`XRAY_CORE_VERSION` / `UPSTREAM_REPO` in [docker/Dockerfile](./docker/Dockerfile)) |
...
2. Create a release on GitHub with the tag of that version (e.g. `v3.3.2`) and keep **Set as the latest release** checked.
   When only the core changes and the node version stays put, add a suffix to the tag (e.g. `v3.3.2-1`).
