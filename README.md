...
Fork of [Remnawave Node](https://github.com/remnawave/node) `3.1.0`, built with the
...
| | |
|---|---|
| Node | `3.1.0` (`version` in `package.json`) |
| Xray core | `v26.8.10` from `Jolymmiles/Xray-core` (`XRAY_CORE_VERSION` / `UPSTREAM_REPO` in [Dockerfile](./Dockerfile)) |
...
2. Create a release on GitHub with the tag of that version (e.g. `3.1.0`) and keep **Set as the latest release** checked.
