...
Fork of [Remnawave Node](https://github.com/remnawave/node) `3.2.0`, built with the
...
| | |
|---|---|
| Node | `3.2.0` (`version` in `package.json`) |
| Xray core | `v26.8.18` from `Jolymmiles/Xray-core` (`XRAY_CORE_VERSION` / `UPSTREAM_REPO` in [docker/Dockerfile](./docker/Dockerfile)) |
...
2. Create a release on GitHub with the tag of that version (e.g. `v3.2.0`) and keep **Set as the latest release** checked.
   When only the core changes and the node version stays put, add a suffix to the tag (e.g. `v3.2.0-1`).
