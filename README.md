...
Fork of the [Remnawave Node](https://github.com/remnawave/node) **`dev` branch**, built with the
...
| | |
|---|---|
| Node | upstream `dev` @ `a6b449e` (`version` in `package.json` still reads `3.2.2`) |
...
1. Bump `version` in `package.json` (and `XRAY_CORE_VERSION` / `UPSTREAM_REPO` in `docker/Dockerfile` when the core changes).
2. Create a release on GitHub with the tag of that version (e.g. `v3.2.2`) and keep **Set as the latest release** checked.
   When only the core changes and the node version stays put, add a suffix to the tag (e.g. `v3.2.2-1`).

   This branch tracks the upstream `dev` branch, so tag it as a pre-release instead — e.g. `v3.2.2-dev.1`
   with **Set as a pre-release** checked. The workflow then publishes `ghcr.io/legiz-ru/remnanode-smux:v3.2.2-dev.1`
   without moving `:latest`, which keeps production nodes on the stable image.
